...
class Point {
  ...
  update(delta) {
    ...
    if (mouse.down) {
      ...
      // Debug: log when mouse is near any point
      // if (dist < 20) {
      //   console.log(
      //     `Mouse near point: dist=${dist.toFixed(1)}, mouse.cut=${
      //       mouse.cut
  ...
  free(constraint) {
    ...
    // Store constraint info before removing for potential repair
    repairHistory.push({
      point: this,
      connectedPoint: constraint.p1 === this ? constraint.p2 : constraint.p1,
      timestamp: Date.now(),
    });
...
class Constraint {
  ...
  resolve() {
    ...
    if (!isRepairing && dist > tearDist) this.p1.free(this);
...
function repairCloth() {
  console.log(
    "Repair cloth clicked! History length:",
    repairHistory.length,
    "Is repairing:",
    isRepairing
  );

  if (repairHistory.length === 0 || isRepairing) {
    console.log("Repair blocked - no history or already repairing");
    return;
  }
  ...
  const repairInterval = setInterval(() => {
    ...
    // Repair the most recent tear
    const repair = repairHistory.pop();
    console.log("Repairing constraint between points");

    // Check if both points still exist and don't already have this constraint
    const hasConstraint = repair.point.constraints.some(
      (c) =>
        (c.p1 === repair.point && c.p2 === repair.connectedPoint) ||
        (c.p2 === repair.point && c.p1 === repair.connectedPoint)
    );

    if (!hasConstraint) {
      repair.point.attach(repair.connectedPoint);
    } else {
